...
const AdminPanel: React.FC<Props> = ({ onClose }) => {
  ...
  const [tab,       setTab]   = useState<'weights' | 'thresholds' | 'system'>('weights');
  ...
  const load = useCallback(async () => {
    try {
      ...
      setConfig(c); setStats(s); setOsm(o);
    } catch (e) {
  ...
  const handleReset = async () => {
    if (!confirm('¿Restaurar configuración de fábrica?')) return;
    try {
  ...
  const WEIGHTS: { key: keyof CalibrationConfig; label: string; color: string }[] = [
    { key: 'weight_population',    label: 'Población (30%)',       color: '#3b82f6' },
    { key: 'weight_mobility',      label: 'Movilidad (25%)',       color: '#8b5cf6' },
    { key: 'weight_commercial',    label: 'Densidad Comercial (25%)', color: '#f97316' },
    { key: 'weight_competition',   label: 'Competencia (15%)',     color: '#14b8a6' },
    { key: 'weight_socioeconomic', label: 'Socioeconómico (5%)',   color: '#ec4899' },
  ];
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-xl max-h-[90vh]
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-gray-700">
          <div>
            ...
            <p className="text-xs text-gray-400">Ajuste de pesos y umbrales del modelo</p>
          </div>
        ...
        <div className="flex border-b border-gray-200 dark:border-gray-700">
          {([['weights', 'Pesos'], ['thresholds', 'Umbrales'], ['system', 'Sistema']] as const).map(([t, label]) => (
              ...
              {stats && (
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    ['Cadenas competidoras', stats.competitor_chains],
